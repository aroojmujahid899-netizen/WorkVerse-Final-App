...
public class ManagerFeedbackActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle s) {
        ...
        if (getSupportActionBar() != null) getSupportActionBar().setTitle("Feedback");
    ...
    private void showSubmitDialog() {
        ...
        EditText etMsg   = new EditText(this); etMsg.setHint("Feedback message..."); etMsg.setMinLines(3);
        ...
        new AlertDialog.Builder(this).setTitle("Submit Feedback").setView(ll)
            .setPositiveButton("Submit", (d, w) -> {
                ...
                Feedback fb = new Feedback(
                    spm.getUid(),
                    spm.getFullName() != null ? spm.getFullName() : "Manager",
                    title, msg, "Manager"
                );
                String sentiment = AIFeedbackAnalyzer.analyzeSentiment(msg);
                fb.setSentiment(sentiment);
                fb.setInsight(AIFeedbackAnalyzer.generateInsight(sentiment, "Medium", 70));
                fb.setAccuracy("Medium");

                FirebaseHelper.getDb().collection(FirebaseHelper.COL_FEEDBACK).add(fb)
                    .addOnSuccessListener(r -> {
                        Toast.makeText(this, "Submitted! AI Sentiment: " + sentiment, Toast.LENGTH_SHORT).show();
                        loadData();
                    })
    ...
    private void loadData() {
        ...
        FirebaseHelper.getDb().collection(FirebaseHelper.COL_FEEDBACK)
            .whereEqualTo("userId", spm.getUid())
