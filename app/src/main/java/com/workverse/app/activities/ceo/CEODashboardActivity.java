...
public class CEODashboardActivity extends AppCompatActivity {
    ...
    TextView tvCEOName, tvTotalEmployees, tvPresentToday, tvTotalManagers, tvAvgKPI;
    LinearLayout qaPerformance, qaAttendance, qaFeedback, qaSales, qaReports, qaNotifications;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (tvCEOName != null)
            tvCEOName.setText(spm.getFullName() != null ? spm.getFullName() : "CEO");
        ...
        qaReports       = findViewById(R.id.qaReports);
        ...
        if (qaPerformance   != null) qaPerformance.setOnClickListener(v   -> go(CEOPerformanceActivity.class));
        if (qaAttendance    != null) qaAttendance.setOnClickListener(v    -> go(CEOAttendanceActivity.class));
        if (qaFeedback      != null) qaFeedback.setOnClickListener(v      -> go(CEOFeedbackActivity.class));
        if (qaSales         != null) qaSales.setOnClickListener(v         -> go(CEOSalesReportActivity.class));
        if (qaReports       != null) qaReports.setOnClickListener(v       -> go(CEOPerformanceActivity.class));
        if (qaNotifications != null) qaNotifications.setOnClickListener(v -> go(CEONotificationsActivity.class));

        if (navNotif   != null) navNotif.setOnClickListener(v   -> go(CEONotificationsActivity.class));
        if (navProfile != null) navProfile.setOnClickListener(v -> go(CEOProfileActivity.class));
    ...
    private void loadStats() {
        ...
        FirebaseHelper.getDb().collection(FirebaseHelper.COL_PERFORMANCE).get()
            .addOnSuccessListener(snap -> {
                if (snap.isEmpty()) { if (tvAvgKPI != null) tvAvgKPI.setText("N/A"); return; }
